...
bool CState::CalAccuOil(CGrid *Grid, double dt){
    ...
    poro = Grid->GetPoro();
    for (int i = 0; i < n_act_cell_; i++){
        ao_[i] = (1 / M)*v[i]*poro[i]*((so_[i] / bo_[i]) - (so_n_[i] / bo_n_[i])) / dt;
	}
    return true;
}
...
bool CState::CalAccuWater(CGrid *Grid, double dt){
    ...
    poro = Grid->GetPoro();
    for (int i = 0; i < n_act_cell_; i++){
        aw_[i] = (1 / M)*v[i]*poro[i]*((sw_[i] / bw_[i]) - (sw_n_[i] / bw_n_[i])) / dt;
    }
    return true;
}
...
bool CState::CalAccuGas(CGrid* Grid, double dt){
    ...
    poro = Grid->GetPoro();
	for (int i = 0; i < n_act_cell_; i++){
        ag_[i] = v[i]*poro[i] / (M*dt)
            *(sg_[i] / bg_[i] - sg_n_[i] / bg_n_[i]
            + so_[i] * rso_[i] / bo_[i] - so_n_[i] * rso_n_[i] / bo_n_[i]);
	}
    return true;
}
...
bool CState::AssembleJacobianOW(CGrid* Grid, double dt, vector<CStandardWell*> &std_well){
	...
	const double* poro; //Pointer of porosity array
	v = Grid->GetVolume();
	poro = Grid->GetPoro();
	...
	//Accumulation Term
	for (int i = 0; i<n_act_cell_; i++){
		jaco_d_[i](0, 0) = -v[i] * sw_[i] * (dporo_dpo_[i] * bw_[i] - dbw_dpw_[i] * poro[i]) / (M*dt*bw_[i] * bw_[i]); //dAw(i)/dPo(i)
		jaco_d_[i](0, 1) = -v[i] * poro[i] / (M*dt*bw_[i]); //dAw(i)/dSw(i)

		jaco_d_[i](1, 0) = -v[i] * so_[i] * (dporo_dpo_[i] * bo_[i] - dbo_dpo_[i] * poro[i]) / (M*dt*bo_[i] * bo_[i]);//dAo(i)/dPo(i)
		jaco_d_[i](1, 1) = v[i] * poro[i] / (M*dt*bo_[i]);// dAo(i)/dSw(i)
	}
...
//	cout << "dt:" << dt << endl;
	...
	const double *poro = Grid->GetPoro();
//	cout<<"V:"<< v[0] <<" Poro:"<<poro[0]<<" bw:"<<bw_[0]<<endl;
	for (int n = 0; n < num_phase_*n_act_cell_; n++){
		if (n % 1 == 0){
			// n is odd, correspond to water
			normalized_resid_w = 5.615 / (v[n / 2] * poro[n / 2])* bw_[n / 2 ]* dt*resid_[n];
...
//				cout <<"Norm: "<< norm_resid <<" "<<fabs(normalized_resid_w) << endl;
			...
			normalized_resid_o = 5.615 / (v[(n - 1) / 2] * poro[(n - 1) / 2])* bo_[(n - 1) / 2] * dt*resid_[n];
...
double CState::CalCFLOW(CGrid* Grid,double dt){
	...
	for (int i = 0; i < n_act_cell_; i++){
		temp = 5.615* outflux[i] * dt / (poro[i] * v[i]);
		if (temp>cfl){
			cfl = temp;
		}
	}
